...
public class DB_List_Nation_Main_Activity extends Activity implements OnScrollListener{
	...
	private class JsonDown extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			dialog = new ProgressDialog(DB_List_Nation_Main_Activity.this);
		...
		protected Void doInBackground(Void... params) {
			...
			jobject = DB_Json.getJURL
					("http://www.ibtk.kr/api_confirm_detail/2b43e497a1327b04b376a603f249df9c?" +
							"model_query=%7B'sb_makingCountry':%7B'$regex':'" +
							nation +
							"'%7D%7D&model_query_pageable=%7B%27pageSize%27:" +
							size +
							"%7D");
			try {
				...
				for(int i = 0; i < jarray.length(); i++) {
					...
					if(nation.equals("인도")) {
						if(jobject.getString("sb_makingCountry").equals("인도")){
							arraylist.add(map);
						}
					} else {
					arraylist.add(map);
		...
		protected void onPostExecute(Void result) {
			...
			adapter = new DB_List_Nation_Adapter(DB_List_Nation_Main_Activity.this, arraylist);
			listview.setAdapter(adapter);
			dialog.dismiss();
			listview.setOnScrollListener(DB_List_Nation_Main_Activity.this);
            if(first == false) {
